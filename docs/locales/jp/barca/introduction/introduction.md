...
ご使用の環境にインストールすると、オペレーティングシステムのコマンドラインから直接**Barca**を実行できます。

<br/>

次のように入力して使用できる**BarcaCLI**コマンドを参照してください。
...
`barca <command> --help`を実行して、コマンド固有のオプションを確認します。

<br/>

以下に、各コマンドの詳細な説明を示します。



| 指示        | それはそう           | 説明  |
| ------------- |:-------------:| -----:|
| `init`      | `i` | 新しいプロジェクトを開始する |
| `add`      | `a`      |   プロジェクトにリソースを追加します |
| `insert`      | `is`      |   プロジェクトに機能を挿入する |
| `integrate` | `in`      |    テクノロジーと統合する |
| `scan` | `sc`      |    スキャンプロジェクト |
| `update` | `up`      |    プロジェクトの更新 |
| `imigrate` | `im`      |    プロジェクトからテクノロジーを移行する |
| `deploy` | `d`      |    プロジェクトを実施する |
